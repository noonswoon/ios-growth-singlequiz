...
static DataController *gInstance = NULL;
...
NSMutableArray *questions; // = [String]()
NSMutableArray *choices; // = [String]()
NSMutableArray *list_questionViewController; // = [QuestionViewController]()

NSDictionary *userInfo; // = Dictionary<String, String>()
...
+ (DataController *) sharedInstance {
    @synchronized(self)
    {
        if (gInstance == NULL) {
            ...
            gInstance.singleLevelResults = [NSMutableArray new];
            [gInstance.singleLevelResults addObject:@"คุณไม่โสดนี่นา (ยินดีด้วย คุณเป็นคนโชคดีคนหนึ่งที่มีใครให้คิดถึง)"];
            [gInstance.singleLevelResults addObject:@"คุณไม่โสดนี่นา (ยินดีด้วย คุณเป็นคนโชคดีคนหนึ่งที่มีใครให้คิดถึง)"];
            [gInstance.singleLevelResults addObject:@"โสดชิว กลับบ้านคนเดียว (ลองหาเพื่อนร่วมทางกลับนะ แชร์ค่าน้ำมัน ประหยัดดี)"];
            [gInstance.singleLevelResults addObject:@"โสดติ๊ส เดินห้างคนเดียว (คราวหลังหาเพื่อนมาสักคน จะได้ช่วยกันถือของ)"];
            [gInstance.singleLevelResults addObject:@"โสดสู้ฟัด หม่ำข้าวนอกบ้านคนเดียว (ลองชวนเพื่อนสักคนมาด้วย เผื่อได้ส่วนลดค่าอาหาร)"];
            [gInstance.singleLevelResults addObject:@"โสดจิต ดูหนังเก้าอี้สวีทคนเดียว (คราวหลังซื้อบัตรธรรมดาก็พอนะ)"];
            [gInstance.singleLevelResults addObject:@"โสดบันเทิง ร้องคาราโอเกะคนเดียว (หาใครมาฟังบ้าง จะได้รู้ว่าควรไปประกวดร้องเพลงรึเปล่า)"];
            [gInstance.singleLevelResults addObject:@"โสดมโน สามารถพูดคนเดียว (ลองหาแอพเพื่อรู้จักและแชทกับคนใหม่ๆดีไหม)"];
            [gInstance.singleLevelResults addObject:@"โสดหลอน เที่ยวสวนสนุกคนเดียว (ลองหาใครสักคนจะได้มีคนคอยลูบหลัง ให้ยาดม)"];
            [gInstance.singleLevelResults addObject:@"โสดสาหัส พายเรือคนเดียว (คิดซะว่าเป็นการออกกำลังกายก็แล้วกัน)"];
            [gInstance.singleLevelResults addObject:@"โสดโ*ตรพ่อ ฉลองวาเลนไทน์คนเดียว (ลองให้โอกาสตัวคุณเองสิ ขอให้เจอคนพิเศษเร็วๆนะ)"];
            
            gInstance.questions                   = [NSMutableArray new]; // = [String]()
            gInstance.choicesList                 = [NSMutableArray new]; // = [String]()
