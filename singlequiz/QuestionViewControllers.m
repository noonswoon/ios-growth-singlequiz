...
#import "QuestionViewControllers.h"
#import "singlequiz-Swift.h"

@interface QuestionViewControllers ()
@end

@implementation QuestionViewControllers
...
- (void) choiceButtonClick:(UIButton *) sender {
    ...
    DataController.summation = DataController.summation + (sender.tag + 1);
        ...
        if (questionNo == DataController.questions.count - 1) {
            ...
            QuestionViewController *viewController = DataController.list_questionViewController[questionNo + 1];
            viewController.modalTransitionStyle = UIModalTransitionStyleFlipHorizontal;
            [self presentViewController:viewController animated:true completion:nil];
        }
}
...
- (void) setQuestionViewController {
    ...
    NSArray *questions = DataController.questions;
    NSArray *choices   = DataController.choices;
    
    CGFloat numberOfChoicePerQuestion = DataController.choices.count/DataController.questions.count;
    CGFloat numberOfQuestion = DataController.questions.count;
    ...
    for (int i=0 ; i<numberOfQuestion ; i++) {
        ...
        for (int j=0 ; j<numberOfChoicePerQuestion ; j++) {
            ...
            NSString *choice = [choices objectAtIndex: (i * numberOfChoicePerQuestion + j)];
        ...
        // NEEDS TO FIX // DataController.list_questionViewController.append( questionViewController )
    }
...
- (void) presentFirstQuestion: (UIViewController*) rootView {
    ...
    QuestionViewController *firstQuestionView  = DataController.list_questionViewController[0];
