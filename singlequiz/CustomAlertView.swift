...
@objc class CustomAlertView: UIView {
    ...
    private func createAlertView() -> UIView {
        if containerView == nil {
            containerView = UIView(frame: CGRectMake(0, 0, 300, 250))
        }
    ...
    override func touchesBegan(touches: Set<NSObject>, withEvent event: UIEvent) {
        ...
        var point = touch.locationInView( alertView )
        var dialogSize = calculateDialogSize()
        
        if (!CGRectContainsPoint(self.bounds, point)) {
            close()
        } else if (point.y > dialogSize.height) {
            close()
        } else if (point.x > dialogSize.width) {
            close()
        }
